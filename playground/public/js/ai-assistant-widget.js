(function () {
    if (window.__HB_AI_WIDGET__) return;
    window.__HB_AI_WIDGET__ = true;

    var STORAGE_KEY = 'hb_ai_widget_conversations_v2';
    var MAX_UI_CONVERSATIONS = 40;
    var MAX_MESSAGES_PER_CONVERSATION = 200;
    var MAX_REQUEST_HISTORY = 50;
    var MAX_DIGEST_LINES = 18;
    ...
    function addProgressMessage(conversationId, text, level, meta) {
        ...
        var collapsibleDetail = normalizeProgressMessageText(row.collapsibleDetail || '');
        ...
        function flushList() {
            ...
            for (j = 0; j < listItems.length; j++) {
                ...
                if (item && typeof item === 'object' && !Array.isArray(item)) {
                    itemText = item.text != null ? String(item.text) : '';
                    childBullets = Array.isArray(item.bullets) ? item.bullets : [];
                } else {
                    itemText = item != null ? String(item) : '';
                }

                var itemHtml = renderInlineMarkdown(itemText);
                if (listType === 'ol' && childBullets.length) {
                    var childRows = [];
                    var k;
                    for (k = 0; k < childBullets.length; k++) {
                        childRows.push('<li>' + renderInlineMarkdown(childBullets[k]) + '</li>');
                    }
                    itemHtml += '<ul>' + childRows.join('') + '</ul>';
                }
                rows.push('<li>' + itemHtml + '</li>');
            }
            html.push('<' + listType + '>' + rows.join('') + '</' + listType + '>');
            listItems = [];
            listType = '';
        }

        function shouldKeepListOnBlankLine(nextIdx) {
            if (!listType || !listItems.length) return false;
            var look = nextIdx;
            while (look < lines.length && !lines[look].trim()) {
                look++;
            }
            if (look >= lines.length) return false;
            var nextLine = lines[look] || '';
            if (listType === 'ol') {
        ...
        for (idx = 0; idx < lines.length; idx++) {
            ...
            if (ulMatch) {
                ...
                if (listType === 'ol' && listItems.length) {
                    ...
                    var lastItem = listItems[lastIdx];
                    if (!lastItem || typeof lastItem !== 'object' || Array.isArray(lastItem)) {
                        lastItem = { text: lastItem != null ? String(lastItem) : '', bullets: [] };
                        listItems[lastIdx] = lastItem;
                    }
                    if (!Array.isArray(lastItem.bullets)) {
                        lastItem.bullets = [];
                    }
                    lastItem.bullets.push(ulMatch[1]);
                    continue;
                }
                if (listType && listType !== 'ul') flushList();
                listType = 'ul';
                listItems.push({ text: ulMatch[1], bullets: [] });
                continue;
            }
            ...
            if (olMatch) {
                ...
                listType = 'ol';
                listItems.push({ text: olMatch[1], bullets: [] });
                continue;
            }
    ...
    function refreshUsageStatusForSelectedProvider(force) {
        ...
        if (!force && state.quotaByProvider && state.quotaByProvider[family]) {
            ...
            renderUsageQuotaStatus();
            return Promise.resolve(state.selectedQuotaStatus);
        }
        ...
        var conversationId = activeConversation && activeConversation.id ? collapseText(activeConversation.id) : '';
        if (conversationId) {
        ...
        if (conversation.messages && typeof conversation.messages.length === 'number') {
            ...
            for (i = start; i < conversation.messages.length; i++) {
                ...
                messages.push({
                    ...
                    collapsibleDetail: normalizeProgressMessageText(msg.collapsibleDetail || ''),
        ...
        if (role === 'assistant') {
            ...
            if (isProgressGroupHeader) {
                ...
                if (role === 'system' && meta && typeof meta === 'object') {
                    collapsibleDetail = normalizeProgressMessageText(meta.collapsibleDetail || '');
    ...
    function applyMergedSystemMessageMeta(target, incoming, incomingTone, incomingMergeKey) {
        ...
        target.collapsibleDetail = normalizeProgressMessageText(incoming.collapsibleDetail || '');
    ...
    function addConversationMessage(role, text, conversationId, options) {
        ...
        var message = {
            ...
            collapsibleDetail: normalizeProgressMessageText(opts.collapsibleDetail || ''),
    ...
    function requestChatViaSseStream(requestOptions, onEvent) {
        ...
        return fetch(apiBase + 'chat_stream', options)
            .then(function (response) {
    ...
    function normalizeBoardSearchRows(payload) {
        ...
        for (i = 0; i < rows.length; i++) {
            var row = rows[i] || {};
            var brdseq = firstNonEmptyText(row.brdseq, row.bid, row.id);
            var btype = firstNonEmptyText(row.btype, row.folder);
            if (!brdseq || !btype) continue;
            var key = btype + '|' + brdseq;
            if (seen[key]) continue;
            seen[key] = true;
            out.push({
                brdseq: brdseq,
    ...
    function buildBoardViewRequests(row) {
        var item = row || {};
        var folder = collapseText(item.btype || '');
        var id = collapseText(item.brdseq || '');
        if (!folder || !id) return [];
        return [{
    ...
    function fetchPagedRowsByRequests(buildRequestsForPage, normalizeRowsFn, params, maxItems, abortSignal, progress, menuLabel) {
        ...
        var label = collapseText(menuLabel || '메뉴');

        progress(
            label + ' AJAX 목록 조회 시작 (limit=' + config.pageSize + ', start_page=' + config.startPage + ', max_pages=' + config.maxPages + ')',
            'detailed'
        );
        ...
        function runPageScan() {
            ...
            progress(
                label + ' 페이지 조회: page=' + currentPage
                + ' (scan ' + (pageIndex + 1) + '/' + config.maxPages + ')'
            ...
            return fetchSearchRowsByRequests(requests, normalize, abortSignal).then(function (result) {
                ...
                if (rows.length) {
                    progress(label + ' 목록 조회 결과: rows=' + rows.length + ' (page=' + currentPage + ')', 'detailed');
                ...
                progress(label + ' 페이지 무결과: page=' + currentPage + ', offset=' + currentOffset, 'detailed');
    ...
    function extractMenuDetailText(payload, rawText) {
        ...
        var i;
        for (i = 0; i < candidates.length; i++) {
            var text = htmlToPlainText(candidates[i] || '');
            if (text) return truncateText(text, 1600);
        }
        var raw = String(rawText || '');
        var plain = htmlToPlainText(raw);
        return plain ? truncateText(plain, 1600) : '';
    }
    ...
    function buildMenuDetailSnapshotContext(title, rows, fetchDetailFn, abortSignal, maxItems) {
        ...
        return mapWithConcurrency(list, 3, function (row) {
            return fetchDetailFn(row, abortSignal).then(function (text) {
                return {
                    row: row || {},
                    text: collapseText(text || '') ? String(text || '') : ''
                };
            });
            ...
            for (i = 0; i < details.length; i++) {
                ...
                out.push('내용: ' + escapeTextForContext(item.text));
            }
            return out.join('\n');
        });
    }
    ...
    function fetchWhisperMenuContext(params, prompt, needDetail, maxItems, abortSignal, progress) {
        ...
        progress(
            '쪽지 대상 범위 확인 (boxes=' + scopeLabel + ', keyword=' + (keyword || '(none)') + ')',
            'detailed'
        );
        return fetchPagedRowsByRequests(
            ...
            progress('쪽지 상세 조회 시작 (items=' + Math.min(rows.length, maxItems) + ')', 'detailed');
    ...
    function fetchCircularMenuContext(params, prompt, needDetail, maxItems, abortSignal, progress) {
        ...
        return fetchPagedRowsByRequests(
            ...
            progress('회람 상세 조회 시작 (items=' + Math.min(rows.length, maxItems) + ')', 'detailed');
    ...
    function fetchApprovalMenuContext(params, prompt, needDetail, maxItems, abortSignal, progress) {
        ...
        progress(
            '전자결재 AJAX 목록 조회 시작 (scopes=' + scopeLabels.join(', ')
            + ', limit=' + maxItems
    ...
    function fetchReportMenuContext(params, prompt, needDetail, maxItems, abortSignal, progress) {
        ...
        progress('업무보고 대상 범위 확인 (scope=' + listTargets.length + ')', 'detailed');
        return fetchPagedRowsByRequests(
            function (pageNo, pageSize) {
                var requests = [];
                var i;
    ...
    function fetchBoardMenuContext(params, prompt, needDetail, maxItems, abortSignal, progress) {
        ...
        var i;
        progress('게시판 대상 범위 확인 (scope=' + scopes.length + ')', 'detailed');
        return fetchPagedRowsByRequests(
            function (pageNo, pageSize) {
                var requests = [];
                var i;
    ...
    function collectSingleBrowserActionContext(action, prompt, abortSignal, onProgress) {
        ...
        var needDetail = toBooleanFlag(params.need_detail) || operation === 'summary' || operation === 'view';
        var maxItems = resolveMenuRequestedItems(params, 10, 30);

        if (menuKey === 'customer') {
            var keyword = sanitizeCustomerKeyword(params.keyword || '');
            var issueKeyword = sanitizeCustomerIssueKeyword(params.issue_keyword || params.issue || '');
            var activityTypes = normalizeCustomerActivityTypesCsv(params.activity_types || params.activityTypes || '');
            var hasPeriod = toBooleanFlag(params.has_period);
            if (!keyword) {
                progress('고객 검색 키워드가 없어 수집을 건너뜁니다.', 'detailed');
                return Promise.resolve({
                    context: '',
                    reason: 'no_context',
            ...
            progress('고객 검색 API 호출: keyword=' + keyword, 'detailed');
            return fetchCustomerSearchContext(keyword, abortSignal).then(function (searchRes) {
                ...
                progress('고객 검색 결과: rows=' + customers.length, 'detailed');
                if (!customers.length || !issueKeyword) {
                    if (!issueKeyword) {
                        progress('이슈 키워드가 없어 고객 활동내역 수집은 생략합니다.', 'detailed');
                    }
                    return {
                ...
                progress('고객 활동내역 API 호출: issue=' + issueKeyword + ', limit=' + activityLimit, 'detailed');
                return fetchCustomerActivityContext(customers, issueKeyword, activityLimit, activityTypes, abortSignal)
                    .then(function (activityRes) {
                        ...
                        progress(
                            '고객 활동내역 결과: matched=' + (parseInt(activity.matchCount, 10) || 0)
                            + ', total=' + (parseInt(activity.totalActivities, 10) || 0)
                            + ', detailFetched=' + (parseInt(activity.detailFetched, 10) || 0),
                            'detailed'
                        );
                        return {
                            context: contexts.join('\n\n'),
        ...
        if (menuKey === 'diary') {
            progress('업무일지 범위 카탈로그 조회 API 호출', 'detailed');
            return fetchDiaryScopeCatalog(abortSignal).then(function (catalog) {
                var contexts = [];
                var scopeContext = buildDiaryScopeCatalogContext(catalog);
                if (scopeContext) contexts.push(scopeContext);
                var tableContext = extractWorklogTableContext();
                if (tableContext) contexts.push(tableContext);
                var deptCount = catalog && Array.isArray(catalog.deptRows) ? catalog.deptRows.length : 0;
                var treeCount = catalog && Array.isArray(catalog.treeNodes) ? catalog.treeNodes.length : 0;
                progress('업무일지 범위 결과: dept_rows=' + deptCount + ', tree_rows=' + treeCount, 'detailed');
                return {
                    context: contexts.join('\n\n'),
                    reason: contexts.length ? '' : 'no_context',
                    stats: {
                        keyword: sanitizeIntentKeyword(params.keyword || '', 120),
                        customer_rows: 0,
                        activity_rows: 0
                    }
                };
        ...
        if (menuKey === 'mail') {
            return fetchMailMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'approval') {
            return fetchApprovalMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'report') {
            return fetchReportMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'schedule') {
            return fetchScheduleMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'project') {
            return fetchProjectMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'board') {
            return fetchBoardMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'todo') {
            return fetchTodoMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'whisper') {
            return fetchWhisperMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'circular') {
            return fetchCircularMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'addrbook') {
            return fetchAddrbookMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        if (menuKey === 'helpdesk') {
            return fetchHelpdeskMenuContext(params, prompt || '', needDetail, maxItems, abortSignal, progress);
        }

        progress('해당 메뉴(' + (menuKey || 'unknown') + ')의 브라우저 AJAX 수집기가 아직 없습니다.', 'detailed');
        return Promise.resolve({
            context: '',
    ...
    function collectBrowserAjaxContextFromActionPlan(actions, prompt, abortSignal, conversationId) {
        ...
        addProgressMessage(conversationId, '[내부 수집] 총 ' + rows.length + '개 액션을 순차 실행합니다.', 'normal');
        ...
        function iterate() {
            ...
            addProgressMessage(
                conversationId,
                '[수집 진행] (' + (index + 1) + '/' + rows.length + ') '
                + actionPlanMenuText(menuKey)
                + ' / ' + actionPlanOperationText(operation)
                + ' / ' + actionPlanSourceText(source),
                'normal'
            );
            index += 1;
            return collectSingleBrowserActionContext(action, prompt || '', abortSignal, function (message, level) {
                addProgressMessage(
                    conversationId,
                    '[세부 진행] ' + actionPlanMenuText(menuKey) + ' - ' + collapseText(message || ''),
                    level || 'detailed'
                );
            })
                .then(function (ctxRes) {
                    ...
                    if (contextText) {
                        var sectionTitle = actionPlanMenuText(menuKey) + ' / '
                            + actionPlanOperationText(operation) + ' / '
                            + actionPlanSourceText(source);
                        ...
                        addProgressMessage(
                            conversationId,
                            '[수집 완료] ' + actionPlanMenuText(menuKey)
                            + ' (customer_rows=' + customerRows + ', activity_rows=' + activityRows + ')',
                            'normal'
                        );
                        ...
                        addProgressMessage(
                            conversationId,
                            '[수집 보류] ' + actionPlanMenuText(menuKey) + ' - ' + actionPlanReasonText(reason),
                            'normal'
                        );
                    }
                    return iterate();
                ...
                .catch(function (err) {
                    ...
                    addProgressMessage(
                        conversationId,
                        '[수집 오류] ' + actionPlanMenuText(menuKey) + ' - ' + truncateText(message, 100),
                        'normal'
                    );
                    return iterate();
                });
    ...
    function normalizeCollectionSections(rawSections, fallbackContext, maxSections, maxSectionChars) {
        ...
        if (!rows.length) {
            var fallback = String(fallbackContext == null ? '' : fallbackContext).trim();
            if (fallback) {
                rows.push({
                    title: 'reference',
                    context: fallback
        ...
        for (i = 0; i < rows.length && out.length < limit; i++) {
            ...
            context = String(context == null ? '' : context).trim();
            if (!context) continue;
    ...
    function extractMailDetailFieldsFromJson(payload) {
        ...
        for (i = 0; i < candidates.length; i++) {
            ...
            var plain = htmlToPlainText(candidate);
    ...
    function extractMailDetailText(html) {
        ...
        try {
            ...
            for (i = 0; i < candidates.length; i++) {
                ...
                if (!node) continue;
                var text = collapseText(node.innerText || node.textContent || '');
                if (text.length > best.length) best = text;
            }
            if (!best) best = htmlToPlainText(source);
            return normalizeMailDetailText(best);
        } catch (e) {
            return normalizeMailDetailText(htmlToPlainText(source));
        }
    }
    ...
    function fetchMailMenuContext(params, prompt, needDetail, maxItems, abortSignal, progress) {
        ...
        progress('메일 메일함 정보 조회 시작', 'detailed');
        return fetchMailBoxInfo(abortSignal).then(function (mailBoxInfo) {
            ...
            progress(
                '메일 목록 조회 시작 (boxes=' + boxCandidates.join(',')
            ...
            function scanMailPages(searchKeyword) {
                ...
                function runPageScan() {
                    ...
                    progress(
                        '메일 페이지 조회: page=' + currentPage
                        + ' (scan ' + (pageIndex + 1) + '/' + scanConfig.maxPages + ')'
                        + ', offset=' + currentOffset
                        + ', limit=' + scanConfig.pageSize,
                        'detailed'
                    );
            ...
            return scanMailPages(keyword).then(function (primaryScan) {
                ...
                progress('메일 키워드 필터 결과 0건, 키워드 없이 재시도', 'detailed');
                ...
                progress('메일 목록 조회 결과: rows=' + items.length, 'detailed');
                ...
                progress('메일 상세 조회 시작 (items=' + Math.min(items.length, maxItems) + ')', 'detailed');
    ...
    function pickCustomerDetailContent(payload) {
        if (!payload || typeof payload !== 'object') return '';
        if (typeof payload.contents === 'string') return htmlToPlainText(payload.contents);
        if (typeof payload.content === 'string') return htmlToPlainText(payload.content);
        if (typeof payload.body === 'string') return htmlToPlainText(payload.body);
        if (typeof payload.comment === 'string') return htmlToPlainText(payload.comment);
        return '';
    }
    ...
    function buildCustomerActivityContext(results, issueKeyword) {
        ...
        for (i = 0; i < rows.length; i++) {
            ...
            for (j = 0; j < acts.length && j < MAX_CUSTOMER_ACTIVITY_ITEMS; j++) {
                ...
                if (act.detailPreview) fields.push('detail_preview=' + escapeTextForContext(act.detailPreview));
    ...
    function buildCompressedHistory(sourceMessages) {
        var rows = [];
        var i9;
        ...
        if (rows.length <= MAX_REQUEST_HISTORY) {
            return { history: rows, digest: '' };
        }

        var recent = rows.slice(-MAX_REQUEST_HISTORY);
        var older = rows.slice(0, rows.length - MAX_REQUEST_HISTORY);
        var start = older.length > MAX_DIGEST_LINES ? older.length - MAX_DIGEST_LINES : 0;
        var lines = [];

        for (i9 = start; i9 < older.length; i9++) {
            var row = older[i9];
            var prefix = row.role === 'assistant' ? 'A' : 'U';
            lines.push(prefix + ': ' + truncateText(row.content, 120));
        }

        var digest = lines.length ? '[Conversation digest - auto compressed]\n' + lines.join('\n') : '';
        return { history: recent, digest: digest };
    }
    ...
    function processRequestItem(item) {
        ...
        var compressed = buildCompressedHistory(historySource);
        ...
        if (compressed.digest) {
            baseContextParts.push(compressed.digest);
        }
        ...
        function buildLlmRequestProgressText(data, evt) {
            ...
            var elapsedMs = parseInt(row.elapsed_ms, 10);
            var elapsedText = '';
            if (!isNaN(elapsedMs) && elapsedMs > 0) elapsedText = ' (' + elapsedMs + 'ms)';
            var errorText = collapseText(row.error || '');
            if (status === 'done') {
                return trff(
                    'assistant_notice_llm_request_done',
                    'LLM 응답 수신: {request}{elapsed}',
                    {
            ...
            if (status === 'error') {
                return trff(
                    'assistant_notice_llm_request_error',
                    'LLM 요청 실패: {request}{error}',
                    {
                        request: requestLabel,
            ...
            return trff(
                'assistant_notice_llm_request_start',
                'LLM 요청: {request}',
                { request: requestLabel }
            );
        }
        ...
        function resolveServerWorkSnapshot(type, evt, data) {
            ...
            if (eventType === 'client_collection_wait') {
                ...
                if (waitStatus === 'server_collecting') {
                    return tr('assistant_notice_server_collection_started', '서버 자료 수집을 시작합니다.');
                }
        ...
        function emitServerWaitPulse(includeDetail) {
            addProgressMessage(
                item.conversationId,
                buildServerWaitPulseText(serverWaitTick * 8, includeDetail === true),
                'normal',
        ...
        function uploadStreamClientCollection(requestId, bundle) {
            ...
            return fetch(apiBase + 'chat_client_collection', options)
                .then(function (res) { return res.json(); })
                .then(function (row) {
                    if (!row || row.success === false) {
                        throw new Error((row && row.msg) ? String(row.msg) : 'client collection upload failed');
                    }
                    return row;
                });
        }
        ...
        function handleStreamClientCollectionRequired(data) {
            ...
            if (!browserActions.length) {
                ...
                collectPromise = collectBrowserAjaxContextFromActionPlan(
                    browserActions,
                    item.prompt || '',
                    requestSignal,
                    item.conversationId
        ...
        function emitServerProgressMessage(evt, type, data) {
            var message = collapseText(evt.message || '');
            var prefixType = type;
            if (type === 'stage') {
                ...
                message = tr('assistant_notice_server_collection_started', '서버 자료 수집을 시작합니다.');
            ...
            if (type !== 'mcp_trace') {
                ...
                if (type === 'collection_route') {
                    ...
                    mergeType = 'llm_request_' + llmStatusForMerge;
                }
        ...
        function applyServerProgressEnvelope(envelope) {
            ...
            if (type === 'action_plan') {
                ...
                if (planActions.length && !serverPlanPreviewShown) {
                    var serverPlanPreview = buildActionPlanPreviewText(planActions, tr('assistant_notice_collection_plan_title_server'));
            ...
            .then(function (res) {
                ...
                if (res && res.success) {
                    if ((!actionPlanBundle.actions || !actionPlanBundle.actions.length) && res.action_plan && Array.isArray(res.action_plan.actions) && res.action_plan.actions.length) {
                        var latePlanPreview = buildActionPlanPreviewText(res.action_plan.actions, tr('assistant_notice_collection_plan_title_server_late'));
                        if (latePlanPreview) {
    ...
    window.HBAiAssistantSetProgressVerbosity = function (level) {
        var normalized = saveProgressVerbosity(level || '');
        state.progressVerbosity = normalized;
        return normalized;
    };

    window.HBAiAssistantGetProgressVerbosity = function () {
        return state.progressVerbosity || 'detailed';
    };

    window.HBAiAssistantSetMcpSseDebug = function (enabled) {
        var normalized = saveMcpSseDebugFlag(enabled);
        state.mcpSseDebug = normalized;
        return normalized;
    };

    window.HBAiAssistantGetMcpSseDebug = function () {
