...
/***/ (function(module, exports) {
    ...
    var tools = {
        ...
        word_filter: function(content) {
           ...
           // ! Notice: content inside comment <!-- ... --> may contain styles for copied table, replace it with empty string may cause the table loses styles.
           // ! So be careful with this line
            ...
            // Convert <u> into <span> with inline-styles for underline
    ...
    ;(function($, tinymce){
        ...
        HanbiroClipboard.prototype = {
                ...
                ed.on('paste', _.bind(function(e){
                    ...
                    if(e.clipboardData)
                    {
                        ...
                        if(types.indexOf('Files') !== -1 && types.indexOf('text/html') === -1) { // is file
                            ...
                            /*
                                * H.Phuc <hoangphuc@hanbiro.vn> - 2022-10-31 - Ticket:  GQ-75332
                                * Issue: Paste text is HTML to editor -> Replace newline by <br> -> Make wrong UI
                                * Solution: If this.editor.checkTextIsHtml = true: if text is HTML -> set for this.html to prevent replace \r\n|\r|\n
                                * e.clipboardData.getData('text/html') -> Copy content from browser to editor
                                * e.clipboardData.getData('text/plain') -> Copy content from notepad/word to editor
                                */
                            if(this.html == '' && this.editor.checkTextIsHtml) {
                                var isHTML = text.match(/(<([^>]+)>)/i);
                                if(isHTML !== null) this.html = text;
                            }
            ...
            _getSanitizeHtml: function() {
                ...
                script.src = `${this.url}/js/sanitize-html.js`;
                document.getElementsByTagName('head')[0].appendChild(script);
            },
    
            replaceHrefToATags: function() {
                // var rex = /(="|blob:|">|>|url\(&quot;|&quot;|url\(\')?(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])?/gi;
                var rex = /(="|<br>|blob:|">|>|url\(&quot;|&quot;|url\(\')?(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])?/gi;
                //Regex : Apply <a> to link text
                // Exception: url in style (Ex: url("image.hanbiro"))\ 
                // ">|> => For normal url from GGSheet  >https://hanbiro 1(*)
                // ="|blob: For IMG 
                // url\(&quot;|&quot;|url\(\') : For style inline
                /**
                 * H.Phuc <hoangphuc@hanbiro.vn> - 2022-07-07 -  @Ticket: GQ-63988
                 * Issue: Paste multi link only format first link
                 * Cause: https://hanbiro.com/<br>https://hanbiro.com/ -> Match with 1(*) because '>' in front of https://hanbiro.com/
                 * -> $1 is exist so it gets raw HTML
                 * Solution: Add catch for <br> and check if $1 != '<br>' -> Format html
                 * -------------------------------------------------------------
                 * H.Phuc <hoangphuc@hanbiro.vn> - 2022-10-20 - Ticket: GQ-71387
                 * Issue: set href with <br> before
                 * Cause: regex get link include <br> (<br>https://blog.naver.com/carlcap/222841068995) so href="<br>link" gets error
                 * Solution: Replace <br> and add it later
                 */
                var add = '';
                this.html = this.html.replace( rex, function ( $0, $1 ) {
                    ...
                    return ($1 && $1 != '<br>' || $0.match(/href=|blob:|">|&quot;|src=|=/)) ? $0 : add + '<a href="' + $0 + '">' + $0 + '</a>';
                });
            },
